...
void LinkedList::Prepend(Bid bid) {
    ...
    //if the list is empty, tail should also point to new node
    if (tail == nullptr) {
        tail = newNode;    
    }
...
void LinkedList::Remove(string bidId) {
    ...
    while (currNode != nullptr && currNode->next != nullptr) {
        //if next node bidId = to curr bid
        if (currNode->next->bid.bidId == bidId) {
            Node *temp = currNode->next; //hold onto next node temp
            currNode->next = currNode->next->next; // make currNode point beyond next node
