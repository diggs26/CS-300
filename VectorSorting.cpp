...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!done) {
        ...
        while (bids.at(low).title < pivotBid.title) {
            low + 1;
        }
        // keep decrementing high index while Pivot < bids[high].title
        while (pivotBid.title > bids.at(high).title) {
            high -1;
        }
        //end while loop
        if (low >= high) {
        ...
        //exiting main loop
            ...
            //swap low and high bids and move them closer to prep for next iteration
            std::swap(bids.at(low), bids.at(high));
            low + 1;
            high - 1;
        }
        }
        //all bids partitioned, return high
        return high;
    }
...
void selectionSort(vector<Bid>& bids) {
    ...
    //if there are no elements, prompt user to load data
    if (result <= 0) {
    ...
    //outer for loop
    //equal to number of elements in vector
    for (auto pos = 0; pos < result; ++pos) {
        // set min to mark where we have sorted the list
        min = pos;

        //inner loop, control comparison of curr value
        for (auto j = pos +1; j < result; j++) {
            ...
            //set min to new index
            if (bids.at(j).title < bids.at(min).title) {
            min = j;
            }

            //swap curr position and index that has curr min value
            if (min != pos) {
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            //initiallize timer variable before calling selection sort
            ticks = clock();

            //call selection sort passing the bids vector
            selectionSort(bids);
        ...
        // FIXME (2b): Invoke the quick sort and report timing results
            ...
            //initialize timer variable before calling quick sort
            ticks = clock();

            //call quick sort passing the bids vector
            quickSort(bids, 0, bids.size()-1);

            //calculate elapsed time and display result
            ticks = clock() - ticks; // curr ticks minus starting ticks
